...
class BlueWhale {
  ...
  static void initialize({
    ...
    final whaleNavigator =
        WhaleNavigator(navigatorKey: navigatorKey, routes: appRoutes);
    ...
    if (kDebugMode) {
      print(
          "BlueWhale initialized. Navigator, Overlays, and i18n (if provided) are ready.");
    }
  }
  ...
  static String tr(String key, BuildContext context) {
    try {
      ...
      if (localePod.isDummy) {
        if (kDebugMode) {
          print(
              "BlueWhale WARNING: Attempting to translate '$key' but WhaleTranslations not provided during BlueWhale.initialize().");
        }
        return key; // Return key if translations are not set up
      }
      final currentLocale =
          localePod.watch(context); // Makes widget reactive to locale changes
      ...
      if (kDebugMode) {
        print(
            "BlueWhale ERROR in tr('$key'): $e. Ensure BlueWhale.initialize() was called with translations, and Reef is accessible.");
      }

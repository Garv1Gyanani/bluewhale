...
class WhaleLocalePod extends WhalePod<Locale> {
  ...
  WhaleLocalePod(Locale initialLocale, {this.isDummy = false})
      : super(initialLocale);

  void setLocale(Locale newLocale) {
    if (isDummy && kDebugMode) {
      print(
          "BlueWhale WARNING: setLocale called, but translations were not provided during BlueWhale.initialize(). Locale change might not reflect in UI via .bwTr().");
    }
    if (value != newLocale) {
      // Only update if the locale is actually different
      value = newLocale;
    }
  }
}

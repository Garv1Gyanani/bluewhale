...
class WhaleTranslations {
  ...
  WhaleTranslations(this.translations,
  ...
  String translate(String key, Locale locale) {
    ...
    if (fallbackCountryCode != null && fallbackCountryCode.isNotEmpty) {
      ...
      if (langMap != null && langMap.containsKey(key)) {
        if (kDebugMode && langCode != fallbackLangCode) {
          print(
              "BlueWhale i18n: Key '$key' not found for locale '$locale'. Using fallback '$fallbackLocale'.");
        }
        return langMap[key]!;
      }
    }
    ...
    if (kDebugMode) {
      print(
          "BlueWhale i18n WARNING: Translation key '$key' not found for locale '$locale' or fallback '$fallbackLocale'. Returning key itself.");
    }

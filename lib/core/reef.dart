...
class _DependencyInjector<T> {
  ...
  _DependencyInjector.singleton(T instance)
      : _factory = null,
        _instance = instance,
        _isSingleton = true,
        _isInitialized = true {
    ...
    if (_isSingleton) {
      if (!_isInitialized) {
        if (_factory == null && _instance == null) {
          throw Exception("BlueWhale Internal Error: Singleton factory/instance is null but not initialized.");
        }
        _instance ??= _factory!();
        _isInitialized = true;
        _notifyInitialized(_instance);
...
class Reef {
  ...
  String _generateKey<T>(String? tag) => tag == null ? T.toString() : '${T.toString()}#$tag';

  void _registerRouteAware(dynamic instance) {
    if (instance is WhaleRouteAwareTank && !_routeAwareTanks.contains(instance)) {
  ...
  void lazyPut<T extends Object>(T Function() factory, {String? tag, bool override = false}) {
    final key = _generateKey<T>(tag);
    if (!override && _dependencies.containsKey(key)) {
      if (kDebugMode) print('WARNING (BlueWhale): Dependency $key already registered. Use override:true if intentional.');
      return;
    }
    if (_dependencies.containsKey(key)) {
      _unregisterRouteAware(_dependencies[key]?._instance);
  ...
  void reset() {
    ...
    if (kDebugMode) print('INFO (BlueWhale): Reef reset. All dependencies cleared.');
  }
}

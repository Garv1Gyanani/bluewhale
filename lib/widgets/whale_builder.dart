...
class _WhaleBuilderState<P extends WhalePod<V>, V>
    extends State<WhaleBuilder<P, V>> implements WhalePodListener {
  late V _value;
  ...
  void _onPodChanged() {
    if (mounted) {
      ...
      } else if (identical(_value, newValue) && _value is List ||
          _value is Map ||
          _value is Set) {
  ...
  void listenTo<T>(WhalePod<T> pod) {
    // WhaleBuilder typically manages its own pod listening, but this
    // allows nested .watch(context) calls to work within its scope.
    if (pod != widget.pod) {
      pod.addListener(_onPodChanged);
    }
  }
  ...
  Widget build(BuildContext context) {
    return WhaleListenerScopeProvider(
      surfaceState:
          null, // We'll update the Provider to accept WhalePodListener
      builderState: this,
      child: widget.builder(context, widget.pod, _value),
    );
  }
}
